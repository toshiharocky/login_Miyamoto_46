...
// 前ページからの変数の受け取り
    ...
    $use_amount = $_SESSION['use_amount'];
...
// 1. SQL文を用意
$stmt = $pdo->prepare(
    "INSERT INTO use_db(id, category, model_num, product_name, use_amount, place, reason, person_in_charge, indate) 
    VALUE (null, :category, :model_num, :product_name, :use_amount, :place, :reason, :person_in_charge, sysdate())"
    );
...
$stmt->bindValue(':use_amount', $use_amount, PDO::PARAM_INT);  //Integer（数値の場合 PDO::PARAM_INT)
...
// ②テーブル「total_db」の「total_amount」「shop_amount」「warehouse_amount」を更新

// A. 「shop_amount」が減少した場合
switch ($place){
  case "店舗":
  // 1. SQL文を用意
      $stmt = $pdo->prepare(
        "UPDATE total_db SET
            total_amount = total_amount - :use_amount,
            shop_amount = shop_amount - :use_amount
  ...
  // 1. SQL文を用意
      $stmt = $pdo->prepare(
        "UPDATE total_db SET
            total_amount = total_amount - :use_amount,
            warehouse_amount = warehouse_amount - :use_amount
            WHERE model_num='$model_num'"
        );
